...
class Users
{
    ...
    //      sendMail($url , $email_user );
        

 
    //  }
    function reset_pw($body, $desemail ){
        $mail             = new PHPMailer(); 
        $mail->IsSMTP();
        $mail->SMTPAuth   = true;
    ...
    function espace_patients()
    {
        ...
        if (!$this->query["tel"]) {
            ApiError::displayError("Merci de renseigner le numéro de téléphone", 515, $this);
        }
        if (!$this->query["pass"]) {
     ...
     function updateTokenForgetPassword($row, $myLocal, $id_users, $token_creation_time, $token){
        ...
        $sql = "UPDATE users SET token_pwd = '$token', token_pwd_creation_time='$token_creation_time' where id = '$id_users'";
        $myLocal->query($sql);
        if ($this->my->errno != 0) {
            ApiError::displayError($this->errorMsg . $this->my->error, 500, $this);
        }
