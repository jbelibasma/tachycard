Uluru.prototype.showAllPatientCard = function () {
    this.logMessage("SHOW PATIENT CARD");
    this.showScreen('find_patient_cards');
    this.findPatientCards();
};

Uluru.prototype.findPatientCards = function () {
    ...
    var success = function (data) {
        ...
        if (data.status === "OK") {
            ...
            if (data.result && data.result.length > 0) {
                this.renderPatientCardsList(data.result);
            } else {
                $("#list_patient_cards").html("Aucune carte trouvée");
            }
            ...
            $("#list_patient_cards").empty().removeClass("visible");
...
Uluru.prototype.renderPatientCardsList = function (data) {
    ...
    content += "<div class='cell'>Indication</div>";
    content += "</div>";
    ...
    data.forEach(function (item) {
        ...
        content += "<div class='cell'>" + item.indication + "</div>";
        content += "</div>";
